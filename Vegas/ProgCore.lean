...
inductive Prog (CB : CmdB) (CS : CmdS) : CtxL → L.Ty → Type where
...
inductive CmdStmtD : CmdS where
  | observe {Γ : CtxL} (cond : L.Expr Γ (L.bool)) : CmdStmtD Γ
...
def evalProg_gen
    {M : Type → Type} (E : Eff M)
    {CB : CmdB} {CS : CmdS}
    (handleBind : {Γ : CtxL} → {τ : L.Ty} → CB Γ τ → EnvL Γ → M (L.Val τ))
    (handleStmt : {Γ : CtxL} → CS Γ → EnvL Γ → M Unit)
    {Γ τ} : Prog  CB CS Γ τ → EnvL Γ → M (L.Val τ)
  | .ret e, env =>
      E.pure (L.eval e env)
  | .letDet e k, env =>
...
def evalWith {CB CS M} (S : LangSem  CB CS M) {Γ τ} :
    Prog  CB CS Γ τ → EnvL Γ → M (L.Val τ) :=
...
@[simp] theorem evalWith_doStmt {CB CS M} (S : LangSem (L := L) CB CS M)
    {Γ τ} (s : CS Γ) (k : Prog  CB CS Γ τ) (env : EnvL Γ) :
...
def evalProgOption {Γ τ} : DProg Γ τ → EnvL Γ → Option (L.Val τ) :=
  evalWith  (DetOptionSem )

def handleObserveOption {Γ} (cond : L.Expr Γ (L.bool)) (env : EnvL Γ) : Option Unit :=
  Eff.guard EffOption (L.toBool (L.eval cond env))
