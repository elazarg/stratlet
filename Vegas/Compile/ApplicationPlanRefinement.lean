...
private theorem environment_preserves_refinement (image : ApplicationImage P L)
    (graph : Graph P L) (native : ApplicationImage.State P L)
    (command : image.application.EnvironmentCommand) (next : ApplicationImage.State P L)
    (hstate : ∃ cfg : Config graph, native.Refines cfg)
    (hnext : next ∈ (image.application.environmentStep native command).support) :
    ∃ cfg : Config graph, next.Refines cfg := by
  obtain ⟨cfg, hrefines⟩ := hstate
  cases command with
  | advance clock =>
...
theorem run_refines (plan : ApplicationPlan accounted fresh build)
  ...
  apply (plan.image deadlineOf).application.run_application_invariant
    ...
    (private_preserves_refinement _ _) _ (environment_preserves_refinement _ _)
