...
theorem behEval_sound {Γ τ} (p : SProg Γ τ) (env : L.Env Γ) (σ : Profile) :
...
theorem toProb_correct {Γ τ} (σ : Profile) (p : SProg Γ τ) (env : L.Env Γ) :
    Prob.evalP (toProb σ p) env = evalS σ p env := by
