...
def deterministicHandler
    (choices : List Nat) : ProtoHandler L where
...
def interactiveHandler : ProtoHandler L where
...
def runInteractive {Γ τ} (p : ProtoProg (L := L) Γ τ)
    (env : L.Env Γ) : IO (L.Val τ) :=
...
theorem runWithSeed_deterministic {Γ τ} (choices : List Nat)
    (p : ProtoProg (L := L) Γ τ) (env : L.Env Γ) :
