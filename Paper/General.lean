...
theorem compiled_runtime_scheduling_boundary
    ...
    (hleft : left ∈ (EventGraph.serializedSystem G hwf hguards).schedules seen)
    (hright : right ∈ (EventGraph.serializedSystem G hwf hguards).schedules seen)
    (hne : left ≠ right) :
    (∀ (state : EventGraph.ReachableConfig G)
        (legal : { joint : ∀ who, Option (EventGraph.FrontierAction G who) //
          (EventGraph.toExecutionProtocol G hwf hguards).Legal state joint }),
        EventGraph.readyInternalNodes G state.1 = ∅ →
          (EventGraph.toExecutionProtocol G hwf hguards).step state legal =
            FinDist.pure (EventGraph.applyFrontier G hwf state legal.1)) ∧
      (∀ (state : (EventGraph.serializedSystem G hwf hguards).State)
          (legal : { joint //
            (EventGraph.serializedSystem G hwf hguards).toExecutionProtocol.Legal
              state joint })
          (next : (EventGraph.serializedSystem G hwf hguards).State),
        next ∈ ((EventGraph.serializedSystem G hwf hguards).toExecutionProtocol.step
            state legal).support →
          EventGraph.readyInternalNodes G next.base.1 = ∅) ∧
      ¬ (EventGraph.serializedSystem G hwf hguards).EnforcesOrder ∧
      (EventGraph.fixedSerializedSystem G hwf hguards).EnforcesOrder :=
  ⟨fun state legal noInternal =>
    ...
    EventGraph.serializedSystem_not_enforcesOrder G hwf hguards
      hleft hright hne,
    EventGraph.fixedSerializedSystem_enforcesOrder G hwf hguards⟩
...
theorem compiled_scheduler_has_no_extra_information
    ...
    (EventGraph.serializedSystem G hwf hguards).SchedulerHasNoExtraInformation :=
  EventGraph.serializedSystem_schedulerHasNoExtraInformation G hwf hguards
...
theorem compiled_scheduler_information_is_player_computable
    ...
    {state : (EventGraph.serializedSystem G hwf hguards).toExecutionProtocol.State}
    (trace : GameTheory.Protocol.ExecutionProtocol.Trace
      (EventGraph.serializedSystem G hwf hguards).toExecutionProtocol state) :
    (EventGraph.serializedSystem G hwf hguards).schedulerInfoFromPlayer
        (fun seen : EventGraph.PublicObservation G ×
          EventGraph.Observation G who => seen.1)
        ((EventGraph.serializedSystem G hwf hguards).revealingSignals.infoOf
          (.player who) trace) =
      (EventGraph.serializedSystem G hwf hguards).revealingSignals.infoOf
...
theorem compiled_permissive_effects_commute
    ...
    (G : EventGraph.Graph Player L) (hwf : G.WF)
    (hguards : EventGraph.GuardLive G) :
    (EventGraph.serializedSystem G hwf hguards).EffectsCommute :=
  EventGraph.serializedSystem_effectsCommute G hwf hguards
...
theorem compiled_serialized_round_implements_atomic
    ...
    {order : List Player}
    (horder : order ∈
      (EventGraph.serializedSystem G hwf hguards).schedules
        (EventGraph.publicObserve G state.1)) :
    EventGraph.applySerializedOrder joint order state =
        EventGraph.applyFrontier G hwf state joint ∧
      (EventGraph.serializedSystem G hwf hguards).resolveOrder
          ((EventGraph.serializedSystem G hwf hguards).withSchedule order joint)
          order state =
        (EventGraph.serializedSystem G hwf hguards).settle
          (EventGraph.applyFrontier G hwf state joint) :=
  ⟨EventGraph.applySerializedOrder_eq_applyFrontier
      G hwf hguards state joint hlegal horder,
    EventGraph.serializedSystem_resolveOrder_eq_settle_atomicFrontier
      G hwf hguards state joint hlegal horder⟩
...
theorem compiled_fixed_order_step_determined_by_players
    ...
    (G : EventGraph.Graph Player L) (hwf : G.WF)
    (hguards : EventGraph.GuardLive G)
    {state : (EventGraph.fixedSerializedSystem G hwf hguards).State}
    {left right : { joint //
      (EventGraph.fixedSerializedSystem G hwf hguards).toExecutionProtocol.Legal
        state joint }}
    (hplayers : ∀ who,
      left.1 (.player who) = right.1 (.player who)) :
    (EventGraph.fixedSerializedSystem G hwf hguards).toExecutionProtocol.step
        state left =
      (EventGraph.fixedSerializedSystem G hwf hguards).toExecutionProtocol.step
        state right :=
  EventGraph.fixedSerializedSystem_step_determined_by_players
...
theorem independent_schedule_signal_preserves_player_nash
    ...
    Participant.IsPlayerNash
        (Participant.IndependentSignal.game source Signal schedulerUtility)
        ((Participant.IndependentSignal.playerDeviationAdequacy
          source Signal schedulerUtility).compileProfile signal profile) ↔
      IsNash source.form (euPreference source.utility) profile :=
  Participant.IndependentSignal.isPlayerNash_iff
...
theorem random_independent_schedule_signal_preserves_player_nash
    ...
    Participant.IsPlayerNash
        (Participant.RandomIndependentSignal.game
          source Signal schedulerUtility)
        (Participant.RandomIndependentSignal.compiledProfile
          source Signal signalLaw profile) ↔
      IsNash source.form (euPreference source.utility) profile :=
  Participant.RandomIndependentSignal.isPlayerNash_iff
...
theorem compiled_serialized_game_wellFormed
  ...
  ⟨program.serializedPerfectRecall,
    ...
    EventGraph.serializedSystem_effectsCommute
      program.graph program.graphWF program.guardLive,
    EventGraph.serializedSystem_schedulerHasNoExtraInformation
      program.graph program.graphWF program.guardLive⟩
...
theorem public_submission_winning_deviation
    ...
    (profile : Profile PublicSubmission.signature) (first : Fin 2)
    (horder : profile .scheduler = FinDist.pure first) :
    expectedUtility (PublicSubmission.game schedulerUtility).utility
      (.player (PublicSubmission.other first))
      ((PublicSubmission.game schedulerUtility).form.play
        (Profile.update profile (.player (PublicSubmission.other first))
          (PublicSubmission.winningPolicy
            (PublicSubmission.other first)))) = 1 :=
  PublicSubmission.winning_deviation schedulerUtility profile first horder
...
theorem public_submission_not_nash
    ...
    (profile : Profile PublicSubmission.signature) (first : Fin 2)
    (horder : profile .scheduler = FinDist.pure first)
    (hpayoff : expectedUtility (PublicSubmission.game schedulerUtility).utility
      (.player (PublicSubmission.other first))
      ((PublicSubmission.game schedulerUtility).form.play profile) = 0) :
    ¬ Participant.IsPlayerNash (PublicSubmission.game schedulerUtility) profile :=
  PublicSubmission.not_nash_of_zero_payoff schedulerUtility profile first horder hpayoff
...
theorem public_submission_no_adequacy
  ...
  PublicSubmission.no_adequacy_of_zero_equilibrium source profile hnash hzero
    schedulerUtility
...
theorem public_submission_approximation_lower_bound
    (schedulerUtility : PublicSubmission.Values → ℝ)
    (profile : GameTheory.Profile PublicSubmission.signature)
    (first : Fin 2) (δ ε : ℝ)
    (horder : profile .scheduler = FinDist.pure first)
    (hpayoff : GameTheory.expectedUtility
      (PublicSubmission.game schedulerUtility).utility
      (.player (PublicSubmission.other first))
      ((PublicSubmission.game schedulerUtility).form.play profile) ≤ δ)
    (hequilibrium : ∀ who replacement,
      GameTheory.expectedUtility (PublicSubmission.game schedulerUtility).utility
        (.player who) ((PublicSubmission.game schedulerUtility).form.play
          (GameTheory.Profile.update profile (.player who) replacement)) ≤
      GameTheory.expectedUtility (PublicSubmission.game schedulerUtility).utility
        (.player who) ((PublicSubmission.game schedulerUtility).form.play profile) + ε) :
    1 ≤ δ + ε :=
  PublicSubmission.approximation_lower_bound schedulerUtility profile first δ ε
