...
strategic frontier and therefore does not exercise adaptive scheduling between player decisions. A separate multistage
abort-aware game is proved in VegasTests/ObservedAbort.lean, including causal execution, public and future chance, and
an exact equilibrium threshold. It is a directly defined strategic kernel, not a VegasCore-syntax compilation.
...
information criterion is necessary and sufficient for refusal-only deviations. The full Nash iff recomputes that
conditional law after every source-strategy deviation. Information refinement cannot decrease exit value.

The module also proves an exact causal implementation law: when the observation is determined at a checkpoint, the
refusal can execute before the future continuation is sampled, preserving the full tagged settlement/abort law.
...
- There is no VegasCore theorem classifying correlated equilibria generated by schedule signals or comparing advance
  and incremental schedule disclosure. The serializer theorem concerns compiled profiles and unilateral deviations.

- Raw-source payoff adequacy is one-way terminal reconstruction. There is no probability-law equality or independent
...
The paper lives in its own Git repository under overleaf/, independently of the Lean repository. Artifact releases need
to pair the paper and code revisions explicitly. The concise draft avoids unverified test counts; published counts
should be generated from the selected artifact revision.

The contribution is an exact, finite, machine-checked compiler interface and its concrete instantiations, not the first
conceptual connection between deviation simulation and equilibrium preservation. Related-work comparisons should
distinguish these model-specific guarantees from cryptographic or end-to-end blockchain compilation results.
